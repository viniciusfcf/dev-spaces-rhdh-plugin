...
echo ""
BACKEND_INTEGRITY_HASH=$(npm pack plugins/dev-spaces-backend/dist-dynamic --pack-destination $DYNAMIC_PLUGIN_ROOT_DIR --json | jq -r '.[0].integrity') &&
    echo "Backend plugin integrity Hash: $BACKEND_INTEGRITY_HASH"

FRONTEND_INTEGRITY_HASH=$(npm pack plugins/dev-spaces/dist-dynamic --pack-destination $DYNAMIC_PLUGIN_ROOT_DIR --json | jq -r '.[0].integrity') &&
